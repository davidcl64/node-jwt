...
function buildToken(options, cb) {
    ...
    var scopSep = options.scopeSep || ' ';
    
    claims.iat = iat;
    
    // If we hace been provided an expiration, we will use it along with delay if necessary
...
function encode(options, cb) {
    options.header = options.header || ALGORITHMS.header(ALGORITHMS.NONE);
    
    // Briefly sanity check paraeters
    if( options.header.alg === ALGORITHMS.NONE && (options.privateKey || options.secret) ) {
        cb(new Error('Invalid Params: assed in a security key but asked for no encryption - which is it?'));
        return;
