...
function encode(options, cb) {
    ...
    if(!options.header) {
        ...
        return;
    } else if( options.privateKey && options.secret ) {
        cb(new Error('Invalid Params: I\'m confused.  You sent me both a privateKey and a secret.'));
        return;
...
function validateSignature(options, cb) {
    ...
    if(options.signature.length > 0) {
        valid = jws.verify(options.signature, options.privateKey || options.secret);
    } else {
        valid = true;
    }
...
function decode(options, cb) {
    ...
    validateSignature({signature: options.token, privateKey: options.privateKey, secret: options.secret}, function(error, result) {
        if(error) {
            cb(error);
            return;
        }
